...
geneCorr <- function(gene='CD274'){
  ...
  for (project in projects) {
    ...
    load(paste0(getwd(), '/data/TCGA_', cancertype, '_exp.rda'))
...
geneSurv <- function(gene='CD274',type='cox') {
  ...
  datasets <- dir('C:/Users/19177/Desktop/Immunotheraypy/tigeR/data')
  datasets <- sub('.Response.Rds', '', datasets[grep('.Response',dir('C:/Users/19177/Desktop/Immunotheraypy/tigeR/data'))])
  data(signatures)

  final_sur_list <- list()
  for (dataset in datasets) {
    exp <- readRDS(paste0(getwd(), '/data/', dataset, '.Response.Rds'))
    ...
    load(file = paste0(getwd(), '/data/', dataset, '.meta.rda'))
